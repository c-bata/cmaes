...
class CMASampler(BaseSampler):
    ...
    def sample_relative(
        ...
        solution_trials = [t for t in completed_trials if optimizer.generation == t.system_attrs.get("cma:generation", 0)]
        if len(solution_trials) >= optimizer.population_size:
            ...
            # Save optimizer when multivariate gaussian distribution is updated.
    ...
    def _restore_or_init_optimizer(
        ...
        for i in reversed(range(len(completed_trials))):
            ...
            optimizer = trial.system_attrs.get("cma:optimizer", None)
            if optimizer is not None:
                return pickle.loads(optimizer)
        ...
        mean = np.array([x0[k] for k in ordered_keys])
